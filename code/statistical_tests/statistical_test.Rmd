...
rank_correlations <- read_csv(
  file="../../data/ESOL/manual_vs_attribution_rank_correlations_around_mean.csv"
) %>% rename(id = ...1)
```
...
Friedman tests will also be used in the analysis of fidelity pos and fidelity 
negative over the attribution methods. Control for the multiple test will be done 
using Bonferonni, resulting in a significance level of $0.05/5 = 0.01$.
...
hypothesis and it is assumed the the method of computing the attribution values 
...
### Wilcoxon-Mann-Whitney test between absolute error class
...
In all attribution methods there is no significance difference between the two 
different absolute error groups (p-values are `r wmw_sme$p.value`, 
`r wmw_shapley$p.value`, `r wmw_hn$p.value` for SME, Shapley and HN respectively
). Therefore, it is concluded that there is not enough evidence in the data to 
