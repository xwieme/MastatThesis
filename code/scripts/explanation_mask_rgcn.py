...
or BRICS substructures. The mask is applied during the RGCN step of the model,
to remove the masked node completely from the graph.
"""
...
if __name__ == "__main__":
    ...
    OUT_DIR_FUNC_GROUP = os.path.join(
        args.DATA_DIR, "attributions_functional_groups_rgcn"
    )
    OUT_DIR_BRICS = os.path.join(args.DATA_DIR, "attributions_brics_rgcn")
    ...
    try:
        ...
        attributions_functional_groups = (
            XAIChem.attribution.substructureMaskExploration(
                models, masks_functional_groups, device, "rgcn"
            )
        )

        if len(masks_functional_groups) > 12:
            raise ValueError("Too many substructures")
        ...
        explanation_df = XAIChem.attribution.substructureMaskExploration(
            models, explanation_df, device, "rgcn"
        )
        ...
        explanation_df = XAIChem.attribution.hamiacheNavarroValue(
            models, explanation_df, 0, device=device, shapley=True, mask_method="rgcn"
        )
