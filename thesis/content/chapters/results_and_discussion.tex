...
This section first discusses the performance of the model, which is followed by
...
methods, two Friedman tests are used (one for each absolute error
category). For testing the presence of a significant difference between
the absolute error (AE) groups three Wilcoxon--Mann--Whitney tests are applied.
Control for the multiple test is done using Bonferonni correction, 
...
does only slightly decrease. As expected, the absolute-negative-fidelity is centered around zero for all attribution 
methods, however, again with long tails. 
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.5]{absolute_fidelity_2}
    \caption{Distribution of fidelity obtained by removing the most positive 
