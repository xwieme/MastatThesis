...
The Hamiache-Navarro value is introduced using one of the simplest examples
(\cref{fig:shapley_hn_example}): 
a three-player game $(N, v, g)$ with $N = \{1, 2, 3\}$ and a graphical structure 
$g = \{\{1, 2\}, \{2, 3\}\}$ defined as a set of adjacent players. Communication 
between players is possible only if these players are connected by the graph 
$\left<N, g\right>$. Players $i$ and $j$ are connected, represented by $i \leftarrow j$, 
if they are adjacent (i.e. $\{i, j\} \in g$) or if there exists a sequence 
$i = i_1, i_2, \dots, i_k = j$ of players such that $\{i_q, i_{q+1}\} \in g$ 
for $1 \le q \le k - 1$. For example, players one and two are adjacent, and 
...
\begin{figure}[h]
    ...
    \caption{Shapley values and HN values for an example game with communication structure 
    given by the shown graph.}
...
Player one can only interact directly with player two and not with player three
in the example game.
...
value of player one. However, the Shapley value would consider the coalition 
$\{1, 3\}$ for a weight of
